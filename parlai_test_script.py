...
TASK_DIRECTORY = os.path.join(get_root_dir(), "mephisto/tasks/shared-mem-chat")

task_title = "Shared Memory Chat Task"
task_description = (
    "This is a simple chat between two people who have shared memory. "
    "In this task, shared memory is previous conversation."
)
...
world_opt = {"num_turns": 5, "turn_timeout": args["turn_timeout"]}
...
db_id = operator.parse_and_launch_run_wrapper(shlex.split(ARG_STRING), extra_args=extra_args)
#logger.info(f"db_id is: {db_id}")
#assignmentsStr = db.find_assignments()
#logger.info(f"assignments: {db.find_assignments()}" )
